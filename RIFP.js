document.addEventListener('DOMContentLoaded', () => {
  ...
  if (sliderContainer) {
    ...
    if (totalSlides <= 1) {
      ...
      sliderContainer.addEventListener('touchstart', e => { isTouch = true; startX = e.touches[0].clientX; }, { passive:true });
      sliderContainer.addEventListener('touchend', e => {
        if (!isTouch) return;
        const dx = e.changedTouches[0].clientX - startX;
        const threshold = 50;
        if (dx > threshold) { currentIndex = (currentIndex - 1 + totalSlides) % totalSlides; }
        else if (dx < -threshold) { currentIndex = (currentIndex + 1) % totalSlides; }
        updateSlider(); isTouch = false;
      });
    }
  }

  /* Flip tarjetas ponentes: click y teclado */
